...
def _build_doc_style_thesis(
    ...
    elif score <  0:  bias, color = "LEANING BEARISH", "#f97316"
    ...
    for key in priority:
        for item in drivers:
            if item not in ordered and key.lower() in item[1].lower():
                ordered.append(item)
    for item in drivers:
...
def render_thesis_page():
    ...
    sig_html = "".join(
        f"<div style='font-size:13px;color:rgba(255,255,255,0.82);margin-bottom:5px;'>{emo} {txt}</div>"
    ...
    risk_html = "".join(
        f"<div style='font-size:12px;color:rgba(255,255,255,0.72);margin-bottom:4px;'>{r}</div>"
        for r in th["risks"]
    )
    ...
    vbd = (
        ...
        + _kv("2σ Daily",  f"{lv['2σ Daily'][0]:,.0f} — {lv['2σ Daily'][1]:,.0f}")
