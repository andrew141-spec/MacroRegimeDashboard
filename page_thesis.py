...
# ══════════════════════════════════════════════════════════════════════════════
# FEAR SCORE 0–21  (7 components × 3 pts each)
# ══════════════════════════════════════════════════════════════════════════════
def _compute_fear_score_021(
    vix_live: float,
    vts_shape: str,
    tail_regime: str,
    vrp_val: float,
    ...
    """
    Score the market from 0 to 21 across 7 components (max 3 pts each).

    Components
    ----------
    1. Vol Regime       — VIX level + term structure shape
    2. Tail Risk        — VVIX/VIX tail regime
    3. Drawdown         — SPY distance from 63d rolling high
    4. SPY-TLT Corr     — 20d rolling correlation (positive = stress)
    5. RV Acceleration  — 5d RV vs 20d RV gap (short-term vol spike)
    6. VRP              — variance risk premium (negative = danger)
    7. VIX Level        — raw VIX absolute level

    Threat levels
    -------------
    0–3   Normal   — business as usual
    4–5   Cautious — something's brewing
    6–9   Elevated — warning signals firing
    10–14 High     — risk-off, reduce size
    15–21 Extreme  — crisis mode
    """
    sa  = _to_1d(spy).reindex(idx).ffill()
    ta  = _to_1d(tlt).reindex(idx).ffill()
    rets_spy = sa.pct_change().dropna()
    rets_tlt = ta.pct_change().reindex(rets_spy.index).dropna()

    # ── 1. Vol Regime (VIX + term structure) ─────────────────────────────
    if vts_shape == "BACKWARDATION" and vix_live >= 20:
        vol_regime_score = 3
    elif vts_shape == "BACKWARDATION" or vix_live >= 22:
        vol_regime_score = 2
    elif vts_shape == "MIXED" or vix_live >= 18:
        vol_regime_score = 1
    else:
        vol_regime_score = 0

    # ── 2. Tail Risk (VVIX/VIX regime) ───────────────────────────────────
    tail_map = {"ELEVATED": 3, "MODERATE": 1, "LOW": 0}
    tail_score = tail_map.get(tail_regime, 1)

    # ── 3. Drawdown (SPY vs 63d rolling high) ────────────────────────────
    roll_high = sa.rolling(63, min_periods=20).max()
    dd = float((sa / roll_high - 1).dropna().iloc[-1]) * 100 if len(sa) > 20 else 0.0
    if dd <= -10:
        dd_score = 3
    elif dd <= -5:
        dd_score = 2
    ...
    # ── 4. SPY-TLT Correlation (20d) ─────────────────────────────────────
    # Positive correlation = bonds not hedging = stress signal
    corr20 = float(rets_spy.rolling(20, min_periods=10).corr(rets_tlt).dropna().iloc[-1]) \
        if len(rets_spy) > 20 else 0.0
    if corr20 >= 0.40:
        corr_score = 3
    elif corr20 >= 0.20:
        corr_score = 2
    ...
    # ── 5. RV Acceleration (5d RV vs 20d RV gap) ─────────────────────────
    rv5  = float(rets_spy.rolling(5,  min_periods=3).std().dropna().iloc[-1] * np.sqrt(252) * 100) \
        if len(rets_spy) > 5 else vix_live
    rv20 = float(rets_spy.rolling(20, min_periods=10).std().dropna().iloc[-1] * np.sqrt(252) * 100) \
        if len(rets_spy) > 20 else vix_live
    rv_gap = rv5 - rv20
    # Gap in top 10% historically ≈ >6pp
    pct_rank_rv = float(current_pct_rank(
        (rets_spy.rolling(5, min_periods=3).std() -
         rets_spy.rolling(20, min_periods=10).std()) * np.sqrt(252) * 100,
        252
    )) if len(rets_spy) > 20 else 50.0
    if pct_rank_rv >= 90:
        rva_score = 3
    elif pct_rank_rv >= 75:
        rva_score = 2
    ...
    # ── 6. VRP (variance risk premium) ───────────────────────────────────
    # Negative VRP = IV below RV = market underpricing risk = danger
    # Strongly positive VRP = expensive protection = fear
    if not np.isfinite(vrp_val):
        vrp_score = 1
    elif vrp_val < -2:
        vrp_score = 3   # IV significantly below RV — complacency/danger
    elif vrp_val < 0:
        vrp_score = 2
    elif vrp_val > 5:
        vrp_score = 2   # very rich IV = fear premium elevated
    elif vrp_val > 2:
        vrp_score = 1
    else:
        vrp_score = 0

    # ── 7. VIX Level (absolute) ───────────────────────────────────────────
    if vix_live >= 30:
        vix_score = 3
    elif vix_live >= 25:
        vix_score = 2
    ...
    total = vol_regime_score + tail_score + dd_score + corr_score + rva_score + vrp_score + vix_score

    if total <= 3:    threat = "NORMAL";   threat_color = "#10b981"
    elif total <= 5:  threat = "CAUTIOUS"; threat_color = "#06b6d4"
    elif total <= 9:  threat = "ELEVATED"; threat_color = "#f59e0b"
    elif total <= 14: threat = "HIGH";     threat_color = "#f97316"
    else:             threat = "EXTREME";  threat_color = "#ef4444"
    ...
    return {
        ...
        "rv_gap": rv_gap,
        "components": [
            {"label": "Vol Regime",      "score": vol_regime_score, "detail": f"VIX {vix_live:.1f} / {vts_shape}"},
            {"label": "Tail Risk",       "score": tail_score,       "detail": tail_regime},
            {"label": "Drawdown",        "score": dd_score,         "detail": f"{dd:+.1f}% from 63d high"},
            {"label": "SPY-TLT Corr",    "score": corr_score,       "detail": f"{corr20:+.2f} (20d)"},
            {"label": "RV Acceleration", "score": rva_score,        "detail": f"5d {rv5:.1f}% vs 20d {rv20:.1f}% (pct {pct_rank_rv:.0f})"},
            {"label": "VRP",             "score": vrp_score,        "detail": f"{vrp_val:+.2f}" if np.isfinite(vrp_val) else "N/A"},
            {"label": "VIX Level",       "score": vix_score,        "detail": f"{vix_live:.1f}"},
        ],
    }


def _render_fear_score_bar(fs: Dict) -> str:
    """
    Returns an HTML string: horizontal bar chart of the 7 components
    styled to match the image — dark background, coloured bars, score labels.
    """
    total     = fs["total"]
    threat    = fs["threat"]
    tc        = fs["threat_color"]
    ...
    for c in comps:
        ...
        pct = (sc / 3) * 100
        # dim if zero
        label_op = "0.9" if sc > 0 else "0.35"
        rows_html += f"""
        <div style='display:grid;grid-template-columns:130px 1fr 40px;align-items:center;
                    gap:6px;margin-bottom:5px;'>
          <div style='font-size:11px;color:rgba(255,255,255,{label_op});
                      font-family:monospace;text-align:right;padding-right:4px;'>
            {c['label']}
          </div>
          <div style='background:rgba(255,255,255,0.06);border-radius:3px;height:14px;
                      position:relative;overflow:hidden;'>
            <div style='position:absolute;left:0;top:0;height:100%;width:{pct}%;
                        background:{bc};border-radius:3px;transition:width 0.3s;'></div>
          </div>
          <div style='font-size:11px;font-family:monospace;font-weight:700;
                      color:{bc if sc>0 else "rgba(255,255,255,0.2)"};text-align:left;
                      padding-left:4px;'>
            {sc}/3
          </div>
        </div>"""
    ...
    # Threat level legend
    levels = [
        ("0–3",   "Normal",   "#10b981", total <= 3),
        ("4–5",   "Cautious", "#06b6d4", 4 <= total <= 5),
        ("6–9",   "Elevated", "#f59e0b", 6 <= total <= 9),
        ("10–14", "High",     "#f97316", 10 <= total <= 14),
        ("15–21", "Extreme",  "#ef4444", total >= 15),
    ]
    legend_html = "<div style='display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;'>"
    for rng, lbl, col, active in levels:
        op = "1" if active else "0.3"
        bdr = f"2px solid {col}" if active else f"1px solid {col}44"
        legend_html += (f"<span style='font-size:10px;font-family:monospace;"
                        f"color:{col};opacity:{op};border:{bdr};"
                        f"border-radius:4px;padding:2px 7px;'>{rng} {lbl}</span>")
    legend_html += "</div>"
    ...
    <div style='display:grid;grid-template-columns:1fr 1fr;gap:0 24px;'>
      <div>
        <div style='display:flex;align-items:baseline;gap:10px;margin-bottom:12px;'>
          <span style='font-size:28px;font-weight:800;color:{tc};font-family:monospace;'>{total}/21</span>
          <span style='font-size:13px;font-weight:700;color:{tc};letter-spacing:0.08em;'>{threat}</span>
          <span style='font-size:10px;color:rgba(255,255,255,0.35);'>Fear Score</span>
        </div>
        {rows_html}
        {legend_html}
      </div>"""
...
def _merton_calibrate(vix: float, vvix: float,
    ...
    lam_base = 0.59; mj_base = -0.026; sj_base = 0.047
    ...
    return {"lam": round(lam,3),"mj": round(mj,4),"sj": round(sj,4),
            "jump_comp": jump_comp,"lam_mult": round(lam_mult,2),
            "vts_adj_used": vts_shape,"sj_mult": round(sj_mult,2)}

def _forward_prob_fig(spx,vix,ndx,vxn,spy,qqq,idx,days=5,n=6000):
    ...
    rv_spx=_rv20(spy); rv_ndx=_rv20(qqq)
    def _sim_paths(spot,ann_vol,days,n):
        sig=ann_vol/100.0; dt=1/252; drift=-0.5*sig**2
    ...
    def _density_grid(paths,spot,days,n_levels=60):
        lo=spot*0.93; hi=spot*1.07; lvls=np.linspace(lo,hi,n_levels); bkt=(hi-lo)/n_levels
        ...
        return Z,lvls,pcts
    spx_paths=_sim_paths(spx,vix,days,n); ndx_paths=_sim_paths(ndx,vxn,days,n)
    ...
    term_spx=spx_paths[:,-1]; term_ndx=ndx_paths[:,-1]
    def _prob_row(paths,spot):
        t=paths[:,-1]
        return {"P(down>3%)":round(np.mean(t<spot*0.97)*100,1),"P(down>2%)":round(np.mean(t<spot*0.98)*100,1),
                "P(down>1%)":round(np.mean(t<spot*0.99)*100,1),"P(flat±1%)":round(np.mean(np.abs(t/spot-1)<0.01)*100,1),
                "P(up>1%)":round(np.mean(t>spot*1.01)*100,1),"P(up>2%)":round(np.mean(t>spot*1.02)*100,1),
                "P(up>3%)":round(np.mean(t>spot*1.03)*100,1),
                "p5":round(float(np.percentile(t,5)),2),"p95":round(float(np.percentile(t,95)),2),
                "p1":round(float(np.percentile(t,1)),2),"median":round(float(np.median(t)),2),
                "expected":round(float(np.mean(t)),2)}
    pr_spx=_prob_row(spx_paths,spx); pr_ndx=_prob_row(ndx_paths,ndx)
    fig=make_subplots(rows=5,cols=2,row_heights=[0.26,0.26,0.20,0.14,0.14],column_widths=[0.72,0.28],
        specs=[[{"type":"heatmap"},{"type":"bar"}],[{"type":"heatmap"},{"type":"bar"}],
               [{"type":"scatter"},{"type":"scatter"}],[{"type":"table"},{"type":"table"}],[{"type":"table"},{"type":"table"}]],
        vertical_spacing=0.04,horizontal_spacing=0.04,
        subplot_titles=[f"SPX Probability Density  |  Spot: {spx:,.2f}  |  VIX: {vix:.1f}%  |  RV(20): {rv_spx:.1f}%",
            "SPX Terminal",f"NDX Probability Density  |  Spot: {ndx:,.2f}  |  VXN: {vxn:.1f}%  |  RV(20): {rv_ndx:.1f}%",
            "NDX Terminal","SPX Scenario Comparison: Median + IQR + 90% CI","NDX Scenario Comparison: Median + IQR + 90% CI",
            "SPX — 1-Week Forward Probability Summary","","NDX — 1-Week Forward Probability Summary",""])
    fig.add_trace(go.Heatmap(z=Z_spx,x=x_labels,y=[f"{p:+.1f}%" for p in pcts_spx],
        colorscale="RdYlGn",reversescale=False,showscale=False,zmin=0),row=1,col=1)
    spot_idx_spx=int(np.argmin(np.abs(lvls_spx-spx)))
    fig.add_hline(y=f"{pcts_spx[spot_idx_spx]:+.1f}%",line_color="white",line_width=1.2,line_dash="solid",row=1,col=1)
    hist_vals,bin_edges=np.histogram(term_spx,bins=40,density=True)
    bin_centers=(bin_edges[:-1]+bin_edges[1:])/2
    fig.add_trace(go.Bar(x=hist_vals,y=[f"{(v/spx-1)*100:+.1f}%" for v in bin_centers],
        orientation="h",marker_color="#06b6d4",opacity=0.7,name="SPX terminal",showlegend=False),row=1,col=2)
    fig.add_trace(go.Heatmap(z=Z_ndx,x=x_labels,y=[f"{p:+.1f}%" for p in pcts_ndx],
        colorscale="RdYlGn",reversescale=False,showscale=False,zmin=0),row=2,col=1)
    fig.add_trace(go.Bar(x=hist_vals,y=[f"{(v/ndx-1)*100:+.1f}%" for v in
        (bin_edges[:-1]+bin_edges[1:])/2/spx*ndx],
        orientation="h",marker_color="#06b6d4",opacity=0.7,name="NDX terminal",showlegend=False),row=2,col=2)
    ...
    for paths,spot_v,row,col in [(spx_paths,spx,3,1),(ndx_paths,ndx,3,2)]:
        ...
        fig.add_trace(go.Scatter(x=list(t_axis)+list(t_axis[::-1]),y=p5+p95[::-1],
            fill="toself",fillcolor="rgba(239,68,68,0.15)",line=dict(color="rgba(0,0,0,0)"),
            name="90% CI",showlegend=(col==1)),row=row,col=col)
        fig.add_trace(go.Scatter(x=list(t_axis)+list(t_axis[::-1]),y=p25+p75[::-1],
            fill="toself",fillcolor="rgba(99,102,241,0.25)",line=dict(color="rgba(0,0,0,0)"),
            name="IQR",showlegend=(col==1)),row=row,col=col)
        fig.add_trace(go.Scatter(x=t_axis,y=p50,line=dict(color="#10b981",width=2),
            name="Median",showlegend=(col==1)),row=row,col=col)
    def _summary_table(pr,spot_v,sym,rv,vix_v):
        exp_chg=(pr["expected"]/spot_v-1)*100; med_chg=(pr["median"]/spot_v-1)*100
        rows=[(f"{sym} Spot",f"{spot_v:,.2f}",""),("Expected (5d)",f"{pr['expected']:,.2f} ({exp_chg:+.2f}%)",""),
              ("Median (5d)",f"{pr['median']:,.2f} ({med_chg:+.2f}%)",""),("","",""),
              ("P(down > 3%)",f"{pr['P(down>3%)']}%","ALERT" if pr["P(down>3%)"]>30 else ("WATCH" if pr["P(down>3%)"]>20 else "")),
              ("P(down > 2%)",f"{pr['P(down>2%)']}%",""),("P(down > 1%)",f"{pr['P(down>1%)']}%",""),
              ("P(flat ±1%)",f"{pr['P(flat±1%)']}%",""),("P(up > 1%)",f"{pr['P(up>1%)']}%",""),
              ("P(up > 2%)",f"{pr['P(up>2%)']}%",""),("P(up > 3%)",f"{pr['P(up>3%)']}%",""),("","",""),
              ...
              ("DV",f"{vix_v:.1f}%",""),("RV(20)",f"{rv:.1f}%",""),("RV(5d)",f"{vix_v*0.85:.1f}%",""),("Skew(60d)","n/a","")]
        return rows
    for pr,spot_v,sym,rv,vix_v,row,col in [(pr_spx,spx,"SPX",rv_spx,vix,4,1),(pr_ndx,ndx,"NDX",rv_ndx,vxn,4,2)]:
        rows=_summary_table(pr,spot_v,sym,rv,vix_v)
        metrics=[r[0] for r in rows]; values=[r[1] for r in rows]; signals=[r[2] for r in rows]
        sig_colors=["#ef4444" if s=="ALERT" else "#f59e0b" if s in ("WATCH","TAIL") else "rgba(0,0,0,0)" for s in signals]
        fig.add_trace(go.Table(
            header=dict(values=["<b>Metric</b>","<b>Value</b>","<b>Signal</b>"],
                fill_color="rgba(30,30,50,0.9)",font=dict(color=["#f59e0b","#f59e0b","#f59e0b"],size=10),align="left",height=20),
            cells=dict(values=[metrics,values,signals],
                fill_color=[["rgba(15,15,25,0.95)"]*len(metrics),["rgba(15,15,25,0.95)"]*len(values),sig_colors],
                font=dict(color=[["rgba(245,158,11,0.9)"]*len(metrics),["rgba(255,255,255,0.85)"]*len(values),
                    ["#ef4444" if s=="ALERT" else "#f59e0b" if s in ("WATCH","TAIL") else "rgba(0,0,0,0)" for s in signals]],
                    size=10,family="monospace"),align="left",height=18)),row=row,col=col)
    ...
    fig.update_layout(margin=dict(t=80,b=20,l=50,r=20),legend=dict(orientation="h",y=0.52,x=0,font=dict(size=10)))
    for r in [1,2]:
        fig.update_xaxes(showgrid=False,row=r,col=1); fig.update_yaxes(showgrid=False,row=r,col=1)
        fig.update_xaxes(showgrid=False,row=r,col=2); fig.update_yaxes(showgrid=False,row=r,col=2)
    return fig

def _ivsurf_from_chain(chain_df,spot,label,vix,vts_shape):
    if chain_df is None or len(chain_df)==0:
        fig=go.Figure(); plotly_dark(fig,title=f"IV Surface — {label} (DATA UNAVAILABLE)",height=420)
        msg=("<b>No NDX/QQQ options chain data available.</b><br>Set GEX Symbol to QQQ or NDX to load this surface."
             if label in ("NDX","QQQ") else
             f"<b>No {label} options chain data available.</b><br>Data loads automatically via yfinance.")
        fig.add_annotation(x=0.5,y=0.5,xref="paper",yref="paper",text=msg,
            showarrow=False,font=dict(color="rgba(255,255,255,0.6)",size=13),align="center")
        return fig
    try:
        from scipy.interpolate import griddata
        from scipy.ndimage import gaussian_filter
        df=chain_df.copy(); df["dte"]=(df["expiry_T"]*365).clip(lower=1); df["moneyness"]=df["strike"]/spot
        df=df[(df["moneyness"]>=0.93)&(df["moneyness"]<=1.15)]
        _dte_limit=10; df=df[(df["dte"]>=1)&(df["dte"]<=_dte_limit)]
        if len(df)<4:
            _dte_limit=21; df=chain_df.copy(); df["dte"]=(df["expiry_T"]*365).clip(lower=1)
            df["moneyness"]=df["strike"]/spot
            df=df[(df["moneyness"]>=0.93)&(df["moneyness"]<=1.15)&(df["dte"]>=1)&(df["dte"]<=_dte_limit)]
        df=df[(df["iv"]>0.005)&(df["iv"]<2.0)]
        iv_med=df["iv"].median(); df=df[(df["iv"]>=iv_med*0.25)&(df["iv"]<iv_med*3.0)]
        if len(df)<6: raise ValueError(f"Only {len(df)} valid chain points")
        _atm_mask=df["moneyness"].between(0.98,1.02)
        atm_iv_est=float(df[_atm_mask]["iv"].mean()*100) if _atm_mask.sum()>0 else vix
        if not np.isfinite(atm_iv_est) or atm_iv_est<=0: atm_iv_est=vix
        hard_lo=max(2.0,atm_iv_est*0.40); hard_hi=min(120.0,atm_iv_est*3.0)
        pts_dte=df["dte"].values.astype(float); pts_m=df["moneyness"].values.astype(float)
        ...
        m_grid=np.linspace(pts_m.min(),pts_m.max(),30); DTE,MON=np.meshgrid(dte_grid,m_grid)
        method="linear" if len(df)>=8 else "nearest"
        Z=griddata(points=np.column_stack([pts_dte,pts_m]),values=pts_iv,
            xi=np.column_stack([DTE.ravel(),MON.ravel()]),method=method,fill_value=np.nan).reshape(DTE.shape)
        nan_mask=~np.isfinite(Z)
        if nan_mask.any():
            Z_nn=griddata(points=np.column_stack([pts_dte,pts_m]),values=pts_iv,
                xi=np.column_stack([DTE.ravel(),MON.ravel()]),method="nearest").reshape(DTE.shape)
            Z[nan_mask]=Z_nn[nan_mask]
        Z=np.clip(Z,hard_lo,hard_hi); Z=gaussian_filter(Z,sigma=1.2); Z=np.clip(Z,hard_lo,hard_hi)
        fig=go.Figure(go.Surface(x=dte_grid,y=m_grid*100,z=Z,
            colorscale=[[0.00,"#0d0221"],[0.20,"#5c1a8a"],[0.45,"#c0392b"],[0.70,"#e67e22"],[0.85,"#f1c40f"],[1.00,"#ffffcc"]],
            showscale=True,colorbar=dict(title="IV%",thickness=14,len=0.75),opacity=0.95,
            contours=dict(z=dict(show=False)),
            lighting=dict(ambient=0.7,diffuse=0.8,roughness=0.5,specular=0.3),lightposition=dict(x=100,y=200,z=0)))
        plotly_dark(fig,title=f"SPX Implied Volatility Surface | Spot: {spot:,.2f} | ATM IV: {atm_iv_est:.1f}%",height=460)
        fig.update_layout(scene=dict(
            xaxis=dict(title="DTE",backgroundcolor="rgba(0,0,0,0)",gridcolor="rgba(255,255,255,0.08)",showbackground=True),
            yaxis=dict(title="Moneyness (%)",backgroundcolor="rgba(0,0,0,0)",gridcolor="rgba(255,255,255,0.08)",showbackground=True),
            zaxis=dict(title="IV%",backgroundcolor="rgba(0,0,0,0)",gridcolor="rgba(255,255,255,0.08)",showbackground=True),
            bgcolor="rgba(0,0,0,0)",camera=dict(eye=dict(x=-1.8,y=-1.5,z=0.8))),margin=dict(l=0,r=0,t=50,b=0))
        return fig
    except Exception as exc:
        fig=go.Figure(); plotly_dark(fig,title=f"IV Surface — {label} (BUILD ERROR)",height=380)
        fig.add_annotation(x=0.5,y=0.5,xref="paper",yref="paper",
            text=f"Surface build failed: {type(exc).__name__}: {exc}",
            showarrow=False,font=dict(color="rgba(255,200,0,0.8)",size=11),align="center")
        return fig

def _ivrv_fig(vix_s,spy,idx):
    from plotly.subplots import make_subplots
    ...
    vrp=(va-rv21).dropna(); dates=va.dropna().index[-252:]
    ...
    rv5_d=rv5.reindex(dates); rv63_d=rv63.reindex(dates); rv21_d=rv21.reindex(dates); va_d=va.reindex(dates)
    fig.add_trace(go.Scatter(x=list(dates)+list(dates[::-1]),
        y=list(rv5_d.fillna(method="ffill"))+list(rv63_d.fillna(method="ffill")[::-1]),
        fill="toself",fillcolor="rgba(6,182,212,0.12)",line=dict(color="rgba(0,0,0,0)"),
        name="5d-63d RV range",showlegend=True),row=1,col=1)
    fig.add_trace(go.Scatter(x=dates,y=va_d,name="VIX (IV)",line=dict(color="#a855f7",width=2)),row=1,col=1)
    fig.add_trace(go.Scatter(x=dates,y=rv21_d,name="21d RV",line=dict(color="#06b6d4",width=2)),row=1,col=1)
    vrp_d=vrp.reindex(dates); colors=["#10b981" if v>=0 else "#ef4444" for v in vrp_d.fillna(0)]
    fig.add_trace(go.Bar(x=dates,y=vrp_d,marker_color=colors,opacity=0.8,name="VRP",showlegend=False),row=2,col=1)
    fig.add_hline(y=0,line_color="rgba(255,255,255,0.3)",line_width=1,row=2,col=1)
    fig.add_trace(go.Scatter(x=[None],y=[None],mode="markers",
        marker=dict(color="#10b981",size=8,symbol="square"),name="IV > RV (premium)"),row=2,col=1)
    fig.add_trace(go.Scatter(x=[None],y=[None],mode="markers",
        marker=dict(color="#ef4444",size=8,symbol="square"),name="IV < RV (discount)"),row=2,col=1)
    ...
    fig.update_layout(legend=dict(orientation="h",y=1.02,x=0),
        ...
        xaxis2=dict(title=""),margin=dict(t=60,b=20,l=60,r=20))
    fig.update_annotations(font_size=11)
    return fig

def _rdist_fig(spy,idx,rd,spot=float("nan")):
    from plotly.subplots import make_subplots
    sa=_to_1d(spy).reindex(idx).ffill(); rets=sa.pct_change().dropna().tail(504)*100
    mu=rd.get("daily_mu",float(rets.mean())); sig=rd.get("daily_sigma",1.0)
    skw=rd.get("skew",0.0); kurt=rd.get("kurtosis",0.0)
    fig=make_subplots(rows=1,cols=2,column_widths=[0.65,0.35],specs=[[{"type":"xy"},{"type":"xy"}]],horizontal_spacing=0.06)
    fig.add_trace(go.Histogram(x=rets,nbinsx=80,name="Actual",marker_color="#6366f1",opacity=0.75,histnorm="probability density"),row=1,col=1)
    xn=np.linspace(float(rets.min())-0.5,float(rets.max())+0.5,300)
    yn=np.exp(-0.5*((xn-mu)/sig)**2)/(sig*np.sqrt(2*np.pi))
    fig.add_trace(go.Scatter(x=xn,y=yn,name=f"Normal(μ={mu:.3f}, σ={sig:.3f})",line=dict(color="#f59e0b",width=2)),row=1,col=1)
    fig.add_vline(x=mu,line_color="#06b6d4",line_width=1.5,annotation_text=f"Mean: {mu:.3f}%",annotation_font_size=10,row=1,col=1)
    for m,col_s in [(1,"#10b981"),(2,"#f59e0b"),(3,"#ef4444")]:
        for s in [-1,1]:
            fig.add_vline(x=s*m*sig+mu,line_dash="dash",line_color=col_s,line_width=1,row=1,col=1)
    ...
    days_2sig=int((np.abs(rets)>2*sig).sum()); pct_2sig=round(days_2sig/len(rets)*100,1)
    fat_tails="YES" if pct_2sig>6.0 else "NO"
    panel_lines=[("GAUSSIAN MEASUREMENTS",None,"#ffffff"),("",None,None),
        (f"SPX Spot:        {_fmt(spot)}",None,"#ffffff"),(f"Daily μ:         {mu:+.4f}%",None,"#e2e8f0"),
        (f"Daily σ:         {sig:.4f}%",None,"#e2e8f0"),(f"Skewness:        {skw:.4f}",None,"#e2e8f0"),
        (f"Excess Kurtosis: {kurt:.4f}",None,"#e2e8f0"),("",None,None),
        ("1-DAY PROBABILITY BANDS",None,"#ffffff"),("",None,None),
        ...
        (f"  99%:       {_fmt(d1_lo99)} — {_fmt(d1_hi99)}",None,"#94a3b8"),("",None,None),
        ("1-WEEK PROBABILITY BANDS",None,"#ffffff"),("",None,None),
        ...
        (f"  99%:       {_fmt(w1_lo99)} — {_fmt(w1_hi99)}",None,"#94a3b8"),("",None,None),
        ("TAIL ANALYSIS",None,"#ffffff"),("",None,None),
        (f"  Days > 2σ:       {pct_2sig}%",None,"#94a3b8"),(f"  (Normal expect:  4.6%)",None,"#64748b"),
    ...
    for i,(text,override_color,base_color) in enumerate(panel_lines):
        ...
        fig.add_annotation(xref="x2 domain",yref="y2 domain",x=0.02,y=1.0-i*y_step,text=text,
            showarrow=False,font=dict(family="monospace",size=11 if is_header else 10,color=color),
            xanchor="left",yanchor="top")
    fig.update_xaxes(visible=False,row=1,col=2); fig.update_yaxes(visible=False,row=1,col=2)
    fig.add_shape(type="rect",xref="x2 domain",yref="y2 domain",x0=0,y0=0,x1=1,y1=1,
        line=dict(color="rgba(255,255,255,0.15)",width=1),fillcolor="rgba(255,255,255,0.03)",row=1,col=2)
    plotly_dark(fig,title=f"SPX Daily Return Distribution (2Y)",height=480)
    fig.update_layout(xaxis=dict(title="Daily Return %",gridcolor="rgba(255,255,255,0.06)"),
        yaxis=dict(title="Density",gridcolor="rgba(255,255,255,0.06)"),
        legend=dict(orientation="v",x=0.36,y=0.99,font=dict(size=10)),
        margin=dict(t=60,b=40,l=60,r=20),bargap=0.02)
    return fig

def _recession_stress(sahm,hy,s2s10_bp,icsa_4w_chg,vix):
    from scipy.special import expit as _sigmoid
    sahm_z=np.clip(sahm/0.4,0,1); hy_z=np.clip((hy-300)/700,0,1)
    curve_z=np.clip((-s2s10_bp-0)/150,0,1); icsa_z=np.clip(icsa_4w_chg/0.20,0,1)
    vix_z=np.clip((vix-15)/35,0,1)
    raw=0.35*sahm_z+0.25*hy_z+0.20*curve_z+0.12*icsa_z+0.08*vix_z
    score=float(_sigmoid((raw-0.5)*8)*100); score=round(np.clip(score,1,99),1)
    if sahm>=0.50 or hy>600:   label="ELEVATED"
    elif sahm>=0.30 or hy>420: label="MODERATE"
    else:                       label="LOW"
    return {"score":score,"label":label,
        "components":{"sahm_contribution":round(0.35*sahm_z*100,1),"hy_contribution":round(0.25*hy_z*100,1),
            "curve_contribution":round(0.20*curve_z*100,1),"icsa_4w_contribution":round(0.12*icsa_z*100,1),
            "vix_contribution":round(0.08*vix_z*100,1)},
        "note":"Recession Stress Index — NOT a calibrated probability."}

def _composite(prob,vrp_val):
    def _bucket_score(p): return (p-50.0)/20.0
    s=_bucket_score(prob.get("tactical_prob",50.0))+_bucket_score(prob.get("short_prob",50.0))+_bucket_score(prob.get("medium_prob",50.0))
...
def _verdict(c,gex):
    ...
    if c<=-2: return (("BEARISH","#ef4444","Negative macro + negative gamma.") if neg else ("LEANING BEARISH","#f97316","Modest bearish lean."))
    if c<=1:
...
def _bands(spot,vix):
    ...
    return {k:round(v,2) for k,v in {"d1lo":spot*(1-dv),"d1hi":spot*(1+dv),"d2lo":spot*(1-2*dv),"d2hi":spot*(1+2*dv),
        "w1lo":spot*(1-wv),"w1hi":spot*(1+wv),"w2lo":spot*(1-2*wv),"w2hi":spot*(1+2*wv)}.items()}

def _news_cats(cat_intel):
    out=[]
    for k,items in cat_intel.items():
        if not items: continue
        m=INTEL_CATEGORIES.get(k,{}); sh=category_shock_score(items)
        out.append({"label":m.get("label",k),"icon":m.get("icon","📰"),"sentiment":round(-(sh-30)/70,4),"count":len(items)})
...
def _sig(e,t): return f"<div style='font-size:12px;margin-bottom:3px;'>{e} {t}</div>"
def _gl(term,defn):
...
# ── GEX Histogram ─────────────────────────────────────────────────────────────
def _gex_histogram(chain_df,spot,flip,max_dte=45,height=360):
    _C_POS="#10b981"; _C_NEG="#ef4444"; _C_FLIP="#f59e0b"
    if chain_df is None or len(chain_df)==0:
        fig=go.Figure(); plotly_dark(fig,title="GEX Histogram (no chain data)",height=height); return fig
    try:
        near_chain=chain_df[chain_df["expiry_T"]<=max_dte/365.0].copy()
        if near_chain.empty: near_chain=chain_df.copy()
        gex_chain=compute_gex_from_chain(near_chain,spot)
        agg=(gex_chain.groupby("strike")["net_gex"].sum().reset_index().sort_values("strike"))
        lo,hi=spot*0.90,spot*1.10; agg=agg[(agg["strike"]>=lo)&(agg["strike"]<=hi)]
        if agg.empty: raise ValueError("No strikes in ±10% range")
        strikes=agg["strike"].tolist(); vals_m=(agg["net_gex"]/1e6).tolist()
        max_abs=max(abs(v) for v in vals_m)
        if max_abs>=5000: vals_display=[v/1000 for v in vals_m]; unit="$B"
        else: vals_display=vals_m; unit="$M"
        colors=[_C_POS if v>0 else _C_NEG for v in vals_display]
        fig=go.Figure(go.Bar(x=strikes,y=vals_display,marker_color=colors,marker_line_width=0,opacity=0.88,name="Net GEX"))
        fig.add_hline(y=0,line_color="rgba(255,255,255,0.30)",line_width=1)
        fig.add_vline(x=spot,line_dash="dot",line_color="rgba(255,255,255,0.75)",line_width=1.5,
            annotation_text=f"SPOT {spot:.0f}",annotation_font_size=10,
            annotation_font_color="rgba(255,255,255,0.85)",annotation_position="top right")
        if flip and lo<flip<hi:
            fig.add_vline(x=flip,line_dash="dash",line_color=_C_FLIP,line_width=1.5,
                annotation_text=f"FLIP {flip:.0f}",annotation_font_size=10,
                annotation_font_color=_C_FLIP,annotation_position="top left")
        x_pad=max(abs(hi-spot),abs(spot-lo))*1.05
        fig.update_layout(xaxis=dict(range=[spot-x_pad,spot+x_pad],title="Strike"),
            yaxis_title=f"Net GEX ({unit})",bargap=0.12,showlegend=False)
        plotly_dark(fig,title=f"GEX Histogram — {unit} per Strike (≤{max_dte} DTE)",height=height)
        return fig
    except Exception as exc:
        fig=go.Figure(); plotly_dark(fig,title=f"GEX Histogram (error: {exc})",height=height); return fig
...
def render_thesis_page():
    ...
    with st.spinner("Loading macro data…"):
        ...
        y3m=r("DGS3MO");y2=r("DGS2");y5=r("DGS5");y10=r("DGS10");y30=r("DGS30")
    ...
    # ── HY OAS + EPU z-scores for 4-component fear composite ─────────────
    _hys_filled=(hys.fillna(hys.dropna().mean()) if hys is not None and hys.dropna().size>0
                 else pd.Series(3.0,index=idx))
    hyz=zscore(_hys_filled)
    _epu_filled=(epu.fillna(epu.dropna().mean()) if epu is not None and epu.dropna().size>0
                 else pd.Series(100.0,index=idx))
    epuz=zscore(_epu_filled)

    # ── 4-component fear composite (for signals) ──────────────────────────
    # VIX 35% · HY OAS 25% · NFCI 25% · EPU 15% — all z-scored
    ...
    nl4wv=_sl(nl4w,0.0); liq_lab="Expanding" if nl4wv>=0 else "Contracting"
    y10_20=y10.diff(20)
    ...
    rec=_stress["score"]; rec_lbl=_stress["label"]
    ...
    stlc63=round(float(sr.rolling(63).corr(tr2).dropna().iloc[-1]),3) if sr.dropna().size>63 else float("nan")
    ...
    y5v=_sl(y5,float("nan")); y2v=_sl(y2,float("nan")); y30v=_sl(y30,float("nan"))
    ...
    # ── Compute 0–21 fear score ───────────────────────────────────────────
    fs = _compute_fear_score_021(
        vix_live=vix_live,
    ...
    # ══════════════════════════════════════════════════════════════════════
    # SECTION 1 — MARKET REGIME (rebuilt)
    # ══════════════════════════════════════════════════════════════════════
    tc = fs["threat_color"]

    # ── Fear score bar chart (left column of card) ────────────────────────
    fear_bar_html = _render_fear_score_bar(fs)

    # ── Macro snapshot (right column of card) ────────────────────────────
    # Yield curve tightness color
    s2s10_col = "#10b981" if s2s10v > 0 else "#ef4444"
    # Stock-bond corr label
    if np.isfinite(stlc63):
        if stlc63 > 0.20:   corr_lbl = "Positive (Inflation)"; corr_col = "#f59e0b"
        elif stlc63 < -0.20: corr_lbl = "Negative (Safe Haven)"; corr_col = "#10b981"
        else:                corr_lbl = "Neutral"; corr_col = "#94a3b8"
    else:
        corr_lbl = "N/A"; corr_col = "#94a3b8"

    macro_snap = f"""
      <div>
        <div style='font-size:10px;color:rgba(255,255,255,0.4);letter-spacing:0.1em;
                    margin-bottom:8px;'>MACRO SNAPSHOT</div>

        <div style='display:flex;align-items:baseline;gap:8px;margin-bottom:8px;'>
          <span style='font-size:15px;font-weight:700;color:{reg_col};'>{macro_reg}</span>
          <span style='font-size:10px;color:rgba(255,255,255,0.35);'>/ {'Loose' if nl4wv>=0 else 'Tight'}</span>
        </div>

        <div style='font-size:10px;color:rgba(255,255,255,0.4);letter-spacing:0.1em;
                    margin-bottom:4px;margin-top:6px;'>YIELDS</div>
        <div style='font-size:12px;font-family:monospace;color:rgba(255,255,255,0.7);margin-bottom:6px;'>
          2Y <b>{y2v:.2f}%</b> &nbsp;|&nbsp; 5Y <b>{y5v:.2f}%</b> &nbsp;|&nbsp;
          10Y <b>{tnxv:.2f}%</b> &nbsp;|&nbsp; 30Y <b>{y30v:.2f}%</b>
          &nbsp;&nbsp;
          <span style='color:{s2s10_col};font-weight:700;'>2s10s: {s2s10v:+.0f}bp</span>
        </div>

        {_kv("Recession P(6m)", f"{rec:.1f}%",
              "#ef4444" if rec>50 else "#f59e0b" if rec>30 else "#10b981")}
        {_kv("Sahm Indicator", f"{sahmv:.3f}",
              "#ef4444" if sahmv>=0.5 else "#f59e0b" if sahmv>=0.3 else "#10b981")}
        {_kv("Growth Z", f"{gzv:+.2f}",
              "#10b981" if gzv>0 else "#ef4444")}
        {_kv("Inflation Z", f"{izv:+.2f}",
              "#f59e0b" if izv>0.5 else "#94a3b8")}
        {_kv("Net Liquidity", f"${abs(nl4wv):.0f}B ({liq_lab})",
              "#10b981" if nl4wv>=0 else "#ef4444")}
        {_kv("CPI MoM Nowcast", f"{cpi_now:+.3f}%" if np.isfinite(cpi_now) else "N/A")}
        {_kv("Stock-Bond Corr 63d", f"{stlc63:.3f} ({corr_lbl})" if np.isfinite(stlc63) else "N/A", corr_col)}
      </div>
    """

    # ── Pill row ──────────────────────────────────────────────────────────
    pills = (
        _pill(f"Regime: {macro_reg}", reg_col)
        + _pill(f"Fear: {fs['total']}/21 {fs['threat']}", tc)
        + _pill(f"Fear Z: {fear_z:+.2f}σ", fc)
        + _pill(f"Liquidity: {liq_lab} (${abs(nl4wv):.0f}B)", "#10b981" if nl4wv>=0 else "#ef4444")
        + _pill(f"Rec P(6m): {rec:.1f}%", "#ef4444" if rec>50 else "#f59e0b" if rec>30 else "#10b981")
    )

    # ── Ticker strip ─────────────────────────────────────────────────────
    strip = ("<div style='display:grid;grid-template-columns:repeat(9,1fr);gap:6px;margin-top:12px;"
    ...
    # ── Assemble Section 1 card ───────────────────────────────────────────
    section1_body = (
        _sh(1, "MARKET REGIME & FEAR SCORE")
        + f"<div style='margin-bottom:10px;'>{pills}</div>"
        + f"<div style='display:grid;grid-template-columns:1fr 1fr;gap:16px 32px;'>"
        + fear_bar_html          # left: 0-21 bar chart (opened in _render_fear_score_bar)
        + macro_snap             # right: macro snapshot
        + "</div></div>"         # close the grid + the left column div opened in _render_fear_score_bar
        + strip
    )
    st.markdown(_card(section1_body, bg=f"{tc}08", border=f"{tc}22"), unsafe_allow_html=True)

    # ── 2. VOLATILITY REGIME ──────────────────────────────────────────────
    vc2=("#ef4444" if vrp["regime"]=="CHEAP" else "#10b981" if vrp["regime"]=="RICH" else "#94a3b8")
    tc2=("#ef4444" if vts["shape"]=="BACKWARDATION" else "#10b981" if vts["shape"]=="CONTANGO" else "#f59e0b")
    ...
    vol=(_sh(2,"VOLATILITY REGIME")
         ...
         +_kv("SPX ATM IV",f"{atm_iv_chain:.1f}%" if (atm_iv_chain and gex_sym in ("SPY","SPX")) else f"{vix_live:.1f}% (VIX proxy)")
         +_kv("NDX ATM IV",f"{atm_iv_chain:.1f}%" if (atm_iv_chain and gex_sym in ("QQQ","NDX")) else f"{q.get('VIX3M_last',vix_live*0.92):.1f}% (VIX3M proxy)")
         +"</div></div>")
    ...
    gleft=("<div>"
           ...
           +_kv("GEX Regime",gex_op_label,gex_rc)+"</div>")
    gright=("<div>"
    ...
    if show_ua:
        gright+=("<div style='margin-top:8px;border-top:1px solid rgba(255,255,255,0.08);padding-top:6px;'>"
                 ...
                 +_kv(f"{ua} Spot",_ua(float(gex_spot)))+_kv(f"{ua} Flip",_ua(flip),"#f59e0b")
                 +_kv(f"{ua} Upper",_ua(upper),"#10b981")+_kv(f"{ua} Lower",_ua(lower),"#ef4444")+"</div>")
    gright+="</div>"
    gex_body=(_sh(3,"GEX LEVELS & DEALER POSITIONING")
              +"<div style='display:grid;grid-template-columns:1fr 1fr;gap:6px 32px;'>"+gleft+gright+"</div>"
    ...
    st.plotly_chart(_gex_histogram(chain_df if chain_df is not None else None,float(gex_spot),float(flip)),use_container_width=True,key="th_gex_hist")
    ...
    with st.spinner("Running simulations…"):
        st.plotly_chart(_forward_prob_fig(spx=spx,vix=vix_live,ndx=ndx,vxn=vxn_live,spy=spy,qqq=qqq,idx=idx),
    ...
    with c6:
        ...
        _ndx_chain=(chain_df if ("chain_df" in dir() and chain_df is not None and gex_sym in ("QQQ","NDX","^NDX")) else None)
    ...
    sigs=(_sig("✅" if vp else "⚠️",f"VRP {'positive' if vp else 'negative'} ({vs2})")
          +_sig("⚠️" if fear_z>0.5 else "✅",f"Fear composite {fl2} ({fear_z:+.2f}σ) · {fs['total']}/21 {fs['threat']}")
    ...
    risks=[]
    if fs["total"]>=6: risks.append(("⚠️",f"Fear score {fs['total']}/21 ({fs['threat']}) — {next((c['label'] for c in fs['components'] if c['score']==3),'multiple signals')} maxed"))
    ...
    with g12:
        st.markdown(_card(
            ...
            +_gl("Fear Score (0–21)",
                 "7 components × 3pts each: Vol Regime, Tail Risk, Drawdown, SPY-TLT Corr, RV Acceleration, VRP, VIX Level. "
                 "0–3 Normal · 4–5 Cautious · 6–9 Elevated · 10–14 High · 15–21 Extreme. "
                 "Used for visual threat assessment. Separate from the z-score composite used in signals.")
            +_gl("Fear Z-Score Composite",
                 "4-component z-score index: VIX (35%) + HY OAS (25%) + NFCI (25%) + EPU (15%). "
                 "All components z-scored over a 1-year rolling window. "
                 ">+1.0σ = elevated fear. <−1.0σ = complacency. Feeds compute_prob_composite.")
    ...
    with g13:
        st.markdown(_card(
            ...
            +_gl("Probability Heatmap","GBM Monte Carlo (n=6,000). Shows probability of SPX/NDX reaching each price level over the next 5 trading days.")
            ...
            +_gl("RV Acceleration","Short-term realized vol (5d) vs medium-term (20d). A 5d spike above 20d in the top 10th percentile historically is an early warning signal for larger drawdowns.")),unsafe_allow_html=True)
