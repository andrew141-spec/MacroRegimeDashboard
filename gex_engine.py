...
def bs_charm(S, K, T, sigma, r=0.05) -> float:
    ...
    Returns the change in delta per YEAR of calendar time.
...
def _gamma_exposure(oi, gamma, spot: float, multiplier=100):
    """Dollar hedge flow for a 1% underlying move."""
    return oi * gamma * multiplier * (spot ** 2) * _ONE_PCT_MOVE


def _vanna_exposure(oi, vanna, spot: float, multiplier=100):
    """Dollar hedge flow for a 1 vol-point IV move."""
    return oi * vanna * multiplier * spot * _ONE_VOL_POINT


def _charm_exposure(oi, charm, spot: float, multiplier=100):
    """Dollar hedge flow for one calendar day of decay."""
    return oi * charm * multiplier * spot * _ONE_DAY
...
def compute_gex_from_chain(chain: pd.DataFrame, spot: float,
    ...
    Units:
      net_gex â€” dollar hedge flow for a 1% underlying move
      net_vex â€” dollar hedge flow for a 1 vol-point IV move
      net_cex â€” dollar hedge flow for one calendar day of decay
    ...
    if has_call_gamma and has_put_gamma:
        ...
        chain["call_gex"] =  _gamma_exposure(chain["call_oi"], call_gamma_col, spot, multiplier)
        chain["put_gex"]  = -_gamma_exposure(chain["put_oi"],  put_gamma_col,  spot, multiplier)
        ...
        chain["call_vex"] =  _vanna_exposure(chain["call_oi"], chain["vanna"], spot, multiplier)
        chain["put_vex"]  = -_vanna_exposure(chain["put_oi"],  chain["vanna"], spot, multiplier)
        chain["net_vex"]  =  chain["call_vex"] + chain["put_vex"]
        chain["call_cex"] =  _charm_exposure(chain["call_oi"], chain["charm"], spot, multiplier)
        chain["put_cex"]  = -_charm_exposure(chain["put_oi"],  chain["charm"], spot, multiplier)
    ...
    chain["call_gex"] =  _gamma_exposure(chain["call_oi"], chain["gamma"], spot, multiplier)
    chain["put_gex"]  = -_gamma_exposure(chain["put_oi"],  chain["gamma"], spot, multiplier)
    chain["net_gex"]  =  chain["call_gex"] + chain["put_gex"]
...
def _legacy_find_gamma_flip(chain: pd.DataFrame) -> float:
    """
...
def _total_gamma_exposure_at_spot(chain: pd.DataFrame, spot: float,
                                  multiplier=100, r=0.05) -> float:
    """Net dealer gamma notional at a hypothetical spot using BS gamma."""
    if chain is None or chain.empty or spot <= 0:
        return np.nan
    gamma = chain.apply(
        lambda row: bs_gamma(spot, row["strike"], row["expiry_T"], row["iv"], r), axis=1
    )
    total = _gamma_exposure(chain["call_oi"].fillna(0), gamma, spot, multiplier)
    total -= _gamma_exposure(chain["put_oi"].fillna(0), gamma, spot, multiplier)
    return float(np.nansum(total))


def find_gamma_flip(chain: pd.DataFrame, spot: float,
                    multiplier=100, r=0.05,
                    search_width: float = 0.15,
                    grid_points: int = 121) -> float:
    """
    Gamma flip = the SPOT LEVEL where total dealer gamma changes sign.

    We estimate it by revaluing BS gamma across a spot grid using the current
    OI and IV surface, then solving for the zero crossing in total net gamma.

    Fallback: if total net gamma never crosses zero in the search band, return
    the spot level with the smallest absolute net gamma.
    """
    if chain is None or chain.empty or spot <= 0:
        return np.nan

    strikes = chain["strike"].dropna().astype(float).values
    if len(strikes) == 0:
        return np.nan

    lo = min(float(np.min(strikes)) * 0.97, spot * (1 - search_width))
    hi = max(float(np.max(strikes)) * 1.03, spot * (1 + search_width))
    grid = np.unique(np.concatenate([
        np.linspace(lo, hi, max(int(grid_points), 25)),
        strikes,
        np.array([spot], dtype=float),
    ]))
    totals = np.array([
        _total_gamma_exposure_at_spot(chain, s, multiplier=multiplier, r=r)
        for s in grid
    ], dtype=float)

    if len(grid) < 2 or not np.isfinite(totals).any():
        return np.nan

    exact = np.where(np.isclose(totals, 0.0, atol=max(1.0, np.nanmax(np.abs(totals)) * 1e-6)))[0]
    if len(exact) > 0:
        best_i = exact[np.argmin(np.abs(grid[exact] - spot))]
        return float(grid[best_i])

    sign_changes = np.where(np.signbit(totals[:-1]) != np.signbit(totals[1:]))[0]
    if len(sign_changes) > 0:
        mids = (grid[:-1] + grid[1:]) / 2.0
        best_i = sign_changes[np.argmin(np.abs(mids[sign_changes] - spot))]
        s1, s2 = float(grid[best_i]),   float(grid[best_i + 1])
        g1, g2 = float(totals[best_i]), float(totals[best_i + 1])
        return s1 + (s2 - s1) * (-g1) / (g2 - g1) if (g2 - g1) != 0 else s1

    nearest_idx = int(np.nanargmin(np.abs(totals)))
    return float(grid[nearest_idx])
...
def build_gamma_state(chain: pd.DataFrame, spot: float, source: str = "yfinance",
    ...
    gex_chain = compute_gex_from_chain(near_chain, spot)
    flip = find_gamma_flip(near_chain, spot)
